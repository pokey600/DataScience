...
def connect_to_sql():
    conn = mysql.connector.connect(user='root', password=input(),
                                  host='127.0.0.1',
                                  database='CNA330')
    return conn
...
def create_tables(cursor, table):
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS jobhunter (JobID Int Primary Key, PostDate DATE, Title TEXT, Location TEXT ,"
                   "Description TEXT,Company TEXT ,Apply_info TEXT, Salary FLOAT, RawMessage TEXT); ")
    return
...
def fetch_new_jobs(arg_dict):
    ...
    query = "https://jobs.github.com/positions.json?" + "location=" + input()  # Add arguments here
    jsonpage = 0
    try:
